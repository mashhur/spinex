...
void loop()
{
  ...
  float accel_angle_z = RADIANS_TO_DEGREES * atan(sqrt(square(accel_y) + square(accel_x)) / accel_z);
  ...
  // old float angle_z = gyro_angle_z;  //Accelerometer doesn't give z-angle
  ...
  */
  if(nCounter > 100)
  {
    Serial.print(F("GYRO_0:"));                // Filtered angle
...
/******************* Flex Sensor data *********************/
  float nAnalogVal = analogRead(0);
  float nFlexVal = map(nAnalogVal, 0, 552, 0, 180);
  //Serial.print(F("Flex_0:"));
  //Serial.print(nFlexVal);
  //Serial.println(F(";"));
/**********************************************************/
  int nBluetooth = mySerial.available();
  char buffer[32];

  if(nBluetooth > 0 && nCounter > 100)
  {
    //Serial.println(F("..........SEND TO BLUETOOTH.........."));
    // to send Gyro values 
    mySerial.write("Gyro_0:");
    
    itoa (angle_x,buffer,10);
    mySerial.write(buffer); mySerial.write(";"); 

    itoa (angle_y,buffer,10);
    mySerial.write(buffer); mySerial.write(";");

    itoa (angle_z,buffer,10);
    mySerial.write(buffer); mySerial.write(";");

    // to send Flex values
    //mySerial.write("Flex_0:");

    //itoa (nFlexVal,buffer,10);
    //mySerial.write(buffer);  mySerial.write(";");
